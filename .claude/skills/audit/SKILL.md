...
Check `API → Services → Core` dependency flow is never reversed:
- Scan imports in `app/core/` — must NOT import from `app/api/` or `app/services/`
- Scan imports in `app/services/` — must NOT import from `app/api/`
- Scan imports in `app/api/` — should only import from `app/services/`, `app/models/`, `app/core/exceptions`
- Check for circular imports between any modules

## 2. Dead Code Audit
- Find unused imports in every `.py` file under `app/`
...
## 3. Concurrency Audit
- Verify ALL `self._device` access in `RelayService` is inside `with self._lock:`
- Check rate limiter middleware for thread-safe counter access
- Verify no shared mutable state between request handlers
- Check for potential deadlocks (nested lock acquisition)

## 4. Observability Audit
- Verify ALL state-changing service methods call `self._audit()`
...
- Check log format consistency: `%(asctime)s | %(levelname)-8s | %(name)s | %(message)s`
- Verify no `print()` statements in `app/`
...
- Verify `ErrorResponse` schema used consistently for all error responses
- Verify static routes before parameterized routes in each router
- Check for missing HTTP status codes in responses dict

## 7. Test Coverage Audit
- For each endpoint in `app/api/v1/`, verify tests exist for:
  - Success path (2xx)
